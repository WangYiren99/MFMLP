...
model = model_generator(opt.method,None).cuda()
...
def train(epoch, logger):
    ...
    # logger.info('===> Epoch: '+str(epoch)+' loss: '+str(epoch_loss / batch_num)+' time: '+str(end-begin))
    return 0
...
def main():
    ...
    logger.info("GPU:{}, Learning rate:{}, batch_size:{}.\n".format(opt.gpu_id,opt.learning_rate, opt.batch_size))
    psnr_max = 0
   
    for epoch in range(start_epoch+1, opt.max_epoch + 1):
        train(epoch, logger)
        (pred, truth, psnr_all, ssim_all, psnr_mean, ssim_mean) = test(epoch, logger)
        # (psnr_mean, ssim_mean) = test(epoch, logger)
        scheduler.step()
        ...
        # name = result_path + '/' + 'Test_{}_{:.2f}_{:.3f}'.format(epoch, psnr_max, ssim_mean) + '.mat'
                ...
                # scio.savemat(name, {'psnr_list': psnr_all, 'ssim_list': ssim_all})
